...
it (`REENTRY_LIVE_ALLOW_UNREVIEWED_DATA=false`). Common direct identifiers in
provider text are also redacted by default; keep
`REENTRY_LIVE_ALLOW_PII=false` unless the deployment has an approved
claimant-data policy.
